...
> 28개 마이크로서비스(Backend 15 + Frontend 12 + Gateway 1)를 단일 엔드포인트로 통합 관리하는 API Gateway
...
```
              ...
              │                   │                    │
     ┌────────┴────────┐ ┌───────┴───────┐  ┌────────┴────────┐
     │  Backend (15)   │ │ Frontend (12) │  │   Standalone    │
     │  :4001 - :4014  │ │ :3001 - :5015 │  │   :8001, :8002  │
     │  :8200          │ │               │  │   :8200         │
     └─────────────────┘ └───────────────┘  └─────────────────┘
```
...
| 섹션 | 설명 |
|------|------|
| `BACKEND_SERVICES` | 15개 백엔드 서비스 정의 (이름, 포트, 경로, 진입점) |
| `FRONTEND_SERVICES` | 12개 프론트엔드 서비스 정의 (이름, 포트, 타입) |
| `API_DOCS` | 주요 서비스별 API 엔드포인트 문서 (deepfake, carelink, cluster 등 7개) |
| `SERVICE_CATEGORIES` | 서비스 카테고리 분류 (AI Platforms, Detection, Healthcare 등 7개 그룹) |
...
| 응답시간 | 미측정 | ms 단위 측정 |
| 프론트엔드 서비스 수 | 15개 (일부 미확인 포트) | 12개 (실제 운영 서비스만) |
...
28개 서비스(Backend 15 + Frontend 12 + Gateway 1)를 순차적으로 시작한다.
...
| Uvicorn | deepfake, a3_adep, a3de, carelink 등 12개 | `uvicorn {module}:app --host 0.0.0.0 --port {port}` |
| Flask-SocketIO | multimodals | `python -c "import app; socketio.run(...)"`|
| Python HTTP | truthlens, webpage_ainex | `python3 -m http.server {port}` |
| Next.js | ainex_home, cluster_master_web 등 6개 | `npx next dev -p {port}` |
| Vite | langgraph_frontend, enterprise_frontend 등 4개 | `npx vite --port {port} --host` |
...
종료 후 28개 포트를 순회하며 아직 사용 중인 포트가 있는지 확인하고 경고를 출력한다.

---

#### `status_services.sh` - 서비스 상태 확인

28개 서비스의 실행 상태를 포트 기반으로 확인하고 색상 표시한다.

**동작:**
1. `ss -tlnp`로 각 서비스 포트 리스닝 여부 확인
2. Backend(15) / Frontend(12) / Gateway(1) 섹션별 표시
...
| `consulting` | AI Consulting Assistant | 4007 | Assistants | 보안 컨설팅 AI 어시스턴트 |
| `factory` | AI Factory | 4008 | Infrastructure | AI 팩토리 엔터프라이즈 생산 시스템 |
| `labor` | AI Labor Compliance | 4009 | Compliance | 노동법 컴플라이언스 AI 시스템 |
| `langgraph` | AgentForge (LangGraph) | 4010 | AI Platforms | 에이전트 워크플로우 플랫폼 |
...
| Key | 서비스명 | 포트 | 타입 | 설명 |
|-----|---------|------|------|------|
| `truthlens` | TruthLens Web | 8001 | Next.js | DeepFake 탐지 웹 인터페이스 |
